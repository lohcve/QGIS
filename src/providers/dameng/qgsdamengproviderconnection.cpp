...
QgsAbstractDatabaseProviderConnection::QueryResult QgsDamengProviderConnection::execSqlPrivate( const QString &sql, bool resolveTypes, QgsFeedback *feedback, std::shared_ptr<QgsPoolDamengConn> dmconn ) const
{
  ...
  else
  {
    ...
    if ( result->ntuples() > 0 )
    {
      ...
      if ( resolveTypes )
      {
        ...
        for ( int rowIdx = 0; rowIdx < numFields; rowIdx++ )
        {
          ...
          static_cast<QgsDamengProviderResultIterator *>( iterator.get() )->mTypeMap[rowIdx] = vType;
        }
      }
    }
    if ( !errCause.isEmpty() )
    ...
    static_cast<QgsDamengProviderResultIterator *>( iterator.get() )->mResult = std::move( res );
  }
  return results;
}
...
QVariantList QgsDamengProviderResultIterator::nextRowPrivate()
{
  ...
  QgsDMResult *res = mResult->result();
...
{
  return mResult ? mResult->DMntuples() : static_cast<long long>( Qgis::FeatureCountState::UnknownCount );
}
